...
extern void LoadInputParameter(char **argv, const unsigned int iRealizNum, const unsigned int iUsdGrid, const unsigned int iFltSegmNum, const unsigned int iFltPtchNum, const unsigned int iBndPtchNum, const unsigned int iFltVertNum, const unsigned int iBndVertNum, const float *fSD_StrssRate, const float *fSD_SlipRate, const float *fSD_SlipRake, unsigned int *iMD_ChgFricBtwEQs, float *fMD_AddNrmStrss, float *fMD_Vp, float *fMD_Vs, float *fMD_Poisson, float *fMD_Lambda, float *fMD_ShearMod, float *fMD_MedDense, unsigned int *iSD_FricLawUSED,  float *fSD_RefStatFric, float *fSD_RefStatFr_vari, float *fSD_RefDynFric, float *fSD_RefDynFr_vari, float *fSD_CritSlipDist, float *fSD_CritSlipD_vari, float *fTDg_CentEpos, float *fTDg_CentNpos, float *fTDg_CentZpos, unsigned int *iTDg_V1, unsigned int *iTDg_V2, unsigned int *iTDg_V3, unsigned int *iTDg_StabType, unsigned int *iTDg_SegID, float *fTDg_SlipRate, float *fTDg_SlipRake, float *fTDg_StrsRteStk, float *fTDg_StrsRteDip, float *fTDg_StatFric, float *fTDg_DynFric, float *fVDg_Epos, float *fVDg_Npos, float *fVDg_Zpos);                        
...
{   if (argc != 7)  {   fprintf(stdout,"Input Error\n Please start the code in the following way:\n\n mpirun -n 4 ./Run_MCQsim InputName RealizationNumber GridNumber WhichDeltT2Use UsePostSeis EQrecordLength\n");        exit(10);           }
    ...
    /*-------------------------------------------------------------------*/
    unsigned int    iUsdGrid,           iDeltT2Use,             iUsePostSeismic,        iRealizNum;
    float           fRecLgth;
    char            *retch;
    /*----------------------------*/
   ...
   // fprintf(stdout,"My Rank: %d  TriNum %d    BASEelem  %d   ADDelem  %d  STARTPOS  %d OFFSET   %d; start: %d     end: %d\n",iRANK, iCmbPtchNum, iBASEelem_A, iADDelem_A, iSTARTPOS_A[iRANK], iOFFSET_A[iRANK], iSTARTPOS_A[iRANK],  (iSTARTPOS_A[iRANK]+iOFFSET_A[iRANK])  );
    ...
    if (iDeltT2Use == 1)    {       fdeltTincr = 1.0*fMeanLegLgth/fMD_Vp[0];                 fdeltTincr = floor(fdeltTincr*100.0)/100.0;            }
    else                    {       fdeltTincr = 1.0E+09;                                                                                           }
    /*-------------------------------------------------------------------*/
    unsigned int    iRandNumber = 1000000; 
    float           fMD_g       = 9.81;
    float           fViscosity  = 1.0E+15; /* in Pa seconds; value doesn't seem right, but need something that corresponds to patch stiffness => to give proper char. time scale for post-seismic decay*/
    ...
    MPI_Allreduce(MPI_IN_PLACE, iTDg_GlobTTmax, 1, MPI_UNSIGNED, MPI_MAX, MPI_COMM_WORLD); 
    
    /*-------------------------------------------------------------------*/ 
    /* once I do include postseismic deformation or after slip, I'll have to add the additional patches here => iFltPtchNum +iBoundPatchNum */
    int          iVectPos;
    ...
    /*the BB matrix can be deleted once the stressing rates are found; however, I still need the "self-stiffness" of the boundary patches
      
      to know how much slip to use for given stress amount in postseismic relaxation*/   
    ...
    /*these two tell me the index for source-receiver pair => how much of STF at source has been "seen" at receiver already...
      takes more memory but is worth it b/c it allows to make the STF shorter and ensures that it will not "blow up" when events are too long...*/
    ...
    MPI_Allreduce(MPI_IN_PLACE, iTempValue, 1, MPI_UNSIGNED, MPI_MAX, MPI_COMM_WORLD);
    ...
    unsigned int    iEQcounter = 0,    iMaxMRFlength  = 10000,      iwritePos,         ireadPos;
    float           fTimeYears,        fMD_CutStrss   = 2000.0; /* stress in Pa ==> earth tides change stress by about 5000Pa (= 0.005MPa); atmospheric pressure is ~100kPa => 2000Pa == 2% of atmospheric pressure change */
    ...
    float         fStkSlip,           fDipSlip,               fUsedStkSlip,       fUsedDipSlip,         fUsedNrmSlip;        
    /*---------------------------------------*/                 
    unsigned int *iEQ_EQongoing;     iEQ_EQongoing    = (unsigned int *) calloc(1 ,sizeof(unsigned int)); 
    unsigned int *iEQ_ChangedStab;   iEQ_ChangedStab  = (unsigned int *) calloc(1, sizeof(unsigned int));       
    unsigned int *iEQ_EQEndCntr;     iEQ_EQEndCntr    = (unsigned int *) calloc(1 ,sizeof(unsigned int));    
    unsigned int *iEQ_ActPtchNum;    iEQ_ActPtchNum   = (unsigned int *) calloc(1, sizeof(unsigned int));  
    unsigned int *iEQ_CmbPtchNum;    iEQ_CmbPtchNum   = (unsigned int *) calloc(1, sizeof(unsigned int));  
    unsigned int *iEQ_MRFlength;     iEQ_MRFlength    = (unsigned int *) calloc(1, sizeof(unsigned int));  
    unsigned int *iEQ_TotalRuptT;    iEQ_TotalRuptT   = (unsigned int *) calloc(1, sizeof(unsigned int));    
    float        *fEQ_SeisPot;       fEQ_SeisPot      = (float *)        calloc(1, sizeof(float));
    
    unsigned int *iEQ_WrtStartPos;   iEQ_WrtStartPos  = (unsigned int *) calloc(iSIZE, sizeof(unsigned int));
    ...
    free(iTempValue);       free(iTDg_GlobTTmax);
    /*-------------------------------------------------------------------*/
    MPI_File fp_MPIout1;
    /*-------------------------------------------------------------------*/
    ...
    {   fTemp             = sqrtf(fTDl_RefStrssRateStk[i]*fTDl_RefStrssRateStk[i] +fTDl_RefStrssRateDip[i]*fTDl_RefStrssRateDip[i]);
        ...
        fTDl_StrssB4_H[i] = fTDl_RefStrssRateStk[i]/fTemp *(fTDl_CurrFric[i]*fTDl_RefNormStrss[i]) -fTemp1*fRandVector[iRandPos[0]]*5.0;       iRandPos[0]++;      if (iRandPos[0] >= iRandNumber) {   iRandPos[0] = rand() % (iRandNumber-1);     }   
        fTemp1            = (fTDl_RefStrssRateDip[i] < 0.0) ?  -1.0 : 1.0;
        fTDl_StrssB4_V[i] = fTDl_RefStrssRateDip[i]/fTemp *(fTDl_CurrFric[i]*fTDl_RefNormStrss[i]) -fTemp1*fRandVector[iRandPos[0]]*5.0;       iRandPos[0]++;      if (iRandPos[0] >= iRandNumber) {   iRandPos[0] = rand() % (iRandNumber-1);     }   
    ...
    {    
        ...
        {   if (iTDl_StabType[i] == 1 )
            {   fTDl_StrssB4_H[i] += fTDl_CurStrssRateStk[i]*fIntSeisTimeStp;
                ...
                {    iEQ_EQongoing[0]     = 1;        iTDl_WasActivated[i]  = 1;          iTDl_t0[i]            = 0;      
        }   }   }
...
/*------------------------------------------------------------------ */
            ...
            {       
                ...
                {    /* determine new friction coefficient */
                    ...
                    {    
                        ...
                        {   
                            fTemp            = sqrtf(fTDlg_STF_H[i*iMaxSTFLength+iTDl_STFcnt[i]]*fTDlg_STF_H[i*iMaxSTFLength+iTDl_STFcnt[i]] + fTDlg_STF_V[i*iMaxSTFLength+iTDl_STFcnt[i]]*fTDlg_STF_V[i*iMaxSTFLength+iTDl_STFcnt[i]]);
                    ...
                    {    
                        ...
                        if (iTDl_WasActivated[i] == 0)          
                        {   iTDl_WasActivated[i] = 1;           iTDl_t0[i]       = iEQ_TotalRuptT[0];              }
                        if (fTemp2 > 0.0)                       
                        {   iEQ_EQongoing[0]     = 1;           iEQ_MRFlength[0] = iEQ_TotalRuptT[0];              
                    ...
                    if (iTDl_WasActivated[i] == 1) 
                    {   /*---------------------------------------------------------*/                         
                        for (j = iFltPtchNum; j--;   )/*taking the receivers..*/
                        {
                            iVectPos = i*iFltPtchNum + j;    
                            /*---------------------------------------------------------*/         
                            ...
                            {   
                                ...
                                if (fTemp > 0.0)
                                {
                                    ...
                                    {   fTemp               = fStkSlip*fTDlg_LocSrcRcv_H[iVectPos] + fDipSlip*fTDlg_LocSrcRcv_V[iVectPos]; /*b/c SrcRcv vector is normalized, this length is already the length of the component in "P-direction" i.e., in SrcRcv direciton*/                
                                        ...
                                        fUsedNrmSlip        = fTDlg_LocSrcRcv_N[iVectPos]*fTemp; /*only need to multiply b/c the SrcRcv vector is normalized -> has length == 1; multiplying gives it length of fTemp*/
                ...
                MPI_Allreduce(MPI_IN_PLACE, fTDg_DeltStrssV, iFltPtchNum , MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
                MPI_Allreduce(MPI_IN_PLACE, fTDg_DeltStrssN, iFltPtchNum , MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
                MPI_Allreduce(MPI_IN_PLACE, iEQ_EQongoing,      1        , MPI_UNSIGNED,MPI_MAX, MPI_COMM_WORLD);
            ...
            for (i = iOFFSET_F[iRANK]; i--;   )   /*here I load the boundary faults, if they are used...; this will then be released via post-seis. deformation (if turned on); otherwise it will not be used...*/
            {   if (iTDl_WasActivated[i] == 1)
                {    for (j = iBndPtchNum; j--;   )
...
/*------------------------------------------------------------------ */
            ...
            for (i = iOFFSET_F[iRANK]; i--;   )   /* now I define all the metrics of the event (its size, its MRF, etc...) */
            {   if (iTDl_WasActivated[i] == 1)     
            ...
            /*------------------------------------------------------------------ */
            MPI_Allreduce(iEQ_ActPtchNum,  iEQ_CmbPtchNum,       1          ,    MPI_UNSIGNED, MPI_SUM, MPI_COMM_WORLD);
            MPI_Allreduce(MPI_IN_PLACE,    iEQ_MRFlength,        1          ,    MPI_UNSIGNED, MPI_MAX, MPI_COMM_WORLD);
            ...
            MPI_Allgather(iEQ_ActPtchNum,      1,      MPI_UNSIGNED,  iEQ_WrtStartPos, 1, MPI_UNSIGNED,  MPI_COMM_WORLD);
            
            for (i = 1; i < iSIZE; i++)        {    iEQ_WrtStartPos[i] += iEQ_WrtStartPos[i-1];             }    
            for (i = iSIZE; i > 0; i--)        {    iEQ_WrtStartPos[i]  = iEQ_WrtStartPos[i-1];             }    
            iEQ_WrtStartPos[0]  = 0;
            ...
            if (iRANK == 0)
            {
                ...
                MPI_File_write_at(fp_MPIout1,   offset1 +2*sizeof(float),                      iEQ_ActPtchNum,       1,            MPI_INT,   &status);
...
/*------------------------------------------------------------------ */    
            ...
            for (i = iOFFSET_F[iRANK]; i--;   ) /* RESET ALL VALUES FOR PATCHES THAT MOVED */
            {
                if (iTDl_WasActivated[i] == 1)     
                {   if (iMD_ChgFricBtwEQs[0] == 0)
                    ...
                    {    
                        fTDl_StatFric[i]   = fSD_RefStatFric[iTDl_SegID[i]]  *(1.0 +(fRandVector[iRandPos[0]]*2.0 -1.0) *fSD_RefStatFr_vari[iTDl_SegID[i]]/100.0);       iRandPos[0]++;      if (iRandPos[0] >= iRandNumber) {   iRandPos[0] = rand() % (iRandNumber-1);     }       
                        fTDl_CurrFric[i]   = fTDl_StatFric[i];
                        fTDl_DynFric[i]    = fSD_RefDynFric[iTDl_SegID[i]]   *(1.0 +(fRandVector[iRandPos[0]]*2.0 -1.0) *fSD_RefDynFr_vari[ iTDl_SegID[i]]/100.0);       iRandPos[0]++;      if (iRandPos[0] >= iRandNumber) {   iRandPos[0] = rand() % (iRandNumber-1);     }  
                        fTDl_Curr_DcVal[i] = fSD_CritSlipDist[iTDl_SegID[i]] *(1.0 +(fRandVector[iRandPos[0]]*2.0 -1.0) *fSD_CritSlipD_vari[iTDl_SegID[i]]/100.0);       iRandPos[0]++;      if (iRandPos[0] >= iRandNumber) {   iRandPos[0] = rand() % (iRandNumber-1);     }      
                    ...
                    for (k = iMaxSTFLength; k--;   )     {  iVectPos = i*iMaxSTFLength + k;         fTDlg_STF_H[iVectPos]  = 0.0;           fTDlg_STF_V[iVectPos]  = 0.0;        }     
                    for (k = iFltPtchNum;   k--;   )     {  iVectPos = i*iFltPtchNum + k;           iTDlg_Pdone[iVectPos] = 0;              iTDlg_Sdone[iVectPos] = 0;          }
                 
                    iEQl_ActPtchID[i]    = 0;               iEQl_t0ofPtch[i]     = 0;               fEQl_DeltTofPtch[i]   = 0.0;            fEQl_SlipHofPtch[i]   = 0.0;
                    fEQl_SlipVofPtch[i]  = 0.0;             iEQl_StabType[i]     = 0;           
                    iTDl_WasActivated[i] = 0;               iTDl_t0[i]           = 0;               iTDl_STFcnt[i]        = 0;     
            ...
            for (i = iSIZE; i--;    )                {      iEQ_WrtStartPos[i]     = 0;                                                                                         }
            for (i = iMaxMRFlength; i--;    )        {      fEQ_MRFvals[i]         = 0.0;           fEQ_MRFvals[i]       = 0.0;                                                 }
            for (j = iBndPtchNum; j--;   )           {      fTDg_TempBnd_H[j]      = 0.0;           fTDg_TempBnd_V[j]    = 0.0;             fTDg_TempBnd_N[j]     = 0.0;        }
            /*------------------------------------------------------------------------- */
            MPI_Allreduce(MPI_IN_PLACE, iEQ_ChangedStab,       1          ,    MPI_UNSIGNED, MPI_MAX, MPI_COMM_WORLD);
                      
            if (iEQ_ChangedStab[0] == 1) /* next, update the current stressing rate*/
            {   
                /*-------------------------------------------------------------------*/
                for (j = iFltPtchNum; j--;    )     {       fTDg_StrssRteChgStk[j]   = 0.0;         fTDg_StrssRteChgDip[j]   = 0.0;         }
                for (i = iOFFSET_F[iRANK]; i--;   )
                {        
                    if (iTDl_StabType[i] != 1)/* if the patch is NOT unstable but instead stable or conditionally stable ==> then it will creep*/
                    {   fTempSlipStk =  fTDl_RefStrssRateStk[i]/fK_FF_SS[iTDl_SelfLoc_F[i]]; /* this is proposed creep (in X/yr) on the not-stick-slipping patches */
