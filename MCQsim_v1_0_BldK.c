...
{  
    ...
    float   fTemp6,             fTemp7,         fTemp8,             fMinDist,           fSrcLgth;
    ...
    {   iSrcTrigs[0] = TR->ivFG_V1_temp[i];                    iSrcTrigs[1] = TR->ivFG_V2_temp[i];                 iSrcTrigs[2] = TR->ivFG_V3_temp[i];
        ...
        {   iGlobPos     = j + MD->ivF_START[MD->iRANK];     
            ...
            if ( (iSrcFltID == iRcvFltID) || (fMinDist > fCutDist1*fSrcLgth) ) //if "self" or distance is larger than 1.0 UsedLgLength's then I can proceed as normal
            {   
                StrainHS_Nikkhoo(fStress, fStressOut, fRcvPt[0], fRcvPt[1], fRcvPt[2], fP1s, fP2s, fP3s, MD->fUnitSlip, 0.0, 0.0, MD->fShearMod, MD->fLambda);         // slip in stk         
                //-----------------------------------------------  
                ...
                //------------------------------------------------------------------------------------------   
                StrainHS_Nikkhoo(fStress, fStressOut, fRcvPt[0], fRcvPt[1], fRcvPt[2], fP1s, fP2s, fP3s, 0.0, MD->fUnitSlip, 0.0, MD->fShearMod, MD->fLambda);         // slip in stk         
                //-----------------------------------------------  
                RotateTensor_inKmatrix(fStressOut, fStress, fvNrm, fvStk, fvDip, 0);
                //-----------------------------------------------  
                fTemp3 = (fStressOut[0]/MD->fUnitSlip)*1.0e-6;           fTemp4 = (fStressOut[1]/MD->fUnitSlip)*1.0e-6;           fTemp5 = (fStressOut[2]/MD->fUnitSlip)*1.0e-6; 
                //------------------------------------------------------------------------------------------   
                if (iGlobPos == i)     {      TR->fvFL_SelfStiffStk[j] = fTemp1;              TR->fvFL_SelfStiffDip[j] = fTemp5;        }       
            } 
        ...
        {   iGlobPos     = j + MD->ivB_START[MD->iRANK];     
            ...
            if (fMinDist > fCutDist1*fSrcLgth) //if "self" or distance is larger than 1.0 UsedLgLength's then I can proceed as normal
			{	//------------------------------------------------------------------------------------------   
            	StrainHS_Nikkhoo(fStress, fStressOut, fRcvPt[0], fRcvPt[1], fRcvPt[2], fP1s, fP2s, fP3s, MD->fUnitSlip, 0.0, 0.0, MD->fShearMod, MD->fLambda);         // slip in stk         
            	//-----------------------------------------------  
            	RotateTensor_inKmatrix(fStressOut, fStress, fvNrm, fvStk, fvDip, 0);
    ...
    {   for (i = 0; i < MD->ivF_OFFSET[MD->iRANK];  i++)   //going through the sources
        {   iGlobPos     = i + MD->ivF_START[MD->iRANK];     
            ...
            {   iRcvTrigs[0] = TR->ivFG_V1_temp[j];                     iRcvTrigs[1] = TR->ivFG_V2_temp[j];                 iRcvTrigs[2] = TR->ivFG_V3_temp[j];               
                ...
                //-----------------------------------------------   
                GetLocKOS_inKmatrix(fvNrm, fvStk, fvDip, fP1r, fP2r, fP3r);   //to rotate into the coordinate system of the receiver!GetLocKOS_inKmatrix(fvNrm, fvStk, fvDip, fP1r, fP2rOut, fP3rOut);   //to rotate into the coordinate system of the receiver!
                //-------------------------------  

                fTemp0 = 0.0;       fTemp1 = 0.0;       fTemp2 = 0.0;           fTemp3 = 0.0;           fTemp4 = 0.0;           fTemp5 = 0.0;           fTemp6 = 0.0;           fTemp7 = 0.0;           fTemp8 = 0.0;      

                if ((iSrcFltID == iRcvFltID) || (fMinDist > fCutDist1*fSrcLgth) )
                {   
                    StrainHS_Nikkhoo(fStress, fStressOut, fRcvPt[0], fRcvPt[1], fRcvPt[2], fP1s, fP2s, fP3s, MD->fUnitSlip, 0.0, 0.0, MD->fShearMod, MD->fLambda);         // slip in stk         
                    //-----------------------------------------------  
                    RotateTensor_inKmatrix(fStressOut, fStress, fvNrm, fvStk, fvDip, 0);
                    ...
                    fTemp3 = (fStressOut[0]/MD->fUnitSlip)*1.0e-6;           fTemp4 = (fStressOut[1]/MD->fUnitSlip)*1.0e-6;           fTemp5 = (fStressOut[2]/MD->fUnitSlip)*1.0e-6; 
                    //------------------------------------------------------------------------------------------   
                    StrainHS_Nikkhoo(fStress, fStressOut, fRcvPt[0], fRcvPt[1], fRcvPt[2], fP1s, fP2s, fP3s, 0.0, 0.0, MD->fUnitSlip, MD->fShearMod, MD->fLambda);         // slip in stk         
                    //-----------------------------------------------  
                    RotateTensor_inKmatrix(fStressOut, fStress, fvNrm, fvStk, fvDip, 0);
                    //-----------------------------------------------  
    ...
    {   iSrcTrigs[0] = TR->ivBG_V1_temp[i];                    iSrcTrigs[1] = TR->ivBG_V2_temp[i];                 iSrcTrigs[2] = TR->ivBG_V3_temp[i];
        ...
        {   iGlobPos     = j + MD->ivB_START[MD->iRANK];     
           ...
           //----------------------------------------------- 
            ...
            if ( (iSrcFltID == iRcvFltID) || (fMinDist >= fCutDist1*fSrcLgth) )    //is "self" or if distance is far enough away to not need the extra distance check 
            {   //------------------------------------------------------------------------------------------   
                StrainHS_Nikkhoo(fStress, fStressOut, fRcvPt[0], fRcvPt[1], fRcvPt[2], fP1s, fP2s, fP3s, MD->fUnitSlip, 0.0, 0.0, MD->fShearMod, MD->fLambda);         // slip in stk         
                //-----------------------------------------------  
                RotateTensor_inKmatrix(fStressOut, fStress, fvNrm, fvStk, fvDip, 0);
